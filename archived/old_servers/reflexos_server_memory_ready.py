...
from reflexos_memory_core import recall_memory, save_memory
...
async def chat_input(request: Request):
    ...
    save_memory(message, user)  # บันทึกลง longterm/shortterm
    return {"response": reply}


@app.post("/recall")
async def recall_memory_entry(request: Request):
    data = await request.json()
    keyword = data.get("keyword", "")
    year = data.get("year", None)
    user = data.get("user", "user_a")
    result = recall_memory(keyword=keyword, user=user, year=year)
