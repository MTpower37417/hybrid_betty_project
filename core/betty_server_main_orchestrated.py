...
from Betty_Ai_GPT_Trainer_Module import gpt_trainer
from betty_prompt_builder_v4 import build_betty_prompt_v4
from flask import Flask, jsonify, request, send_file
from reflex_prompt_orchestrator import load_persona
...
class IntegratedServer:
    ...
    def setup_routes(self):
        @self.app.route('/')
        def index():
            return send_file('index.html')  # ใช้ไฟล์จาก root directory
        ...
        def chat():
            ...
            memory_context = self.memory_core.get_relevant_memories(message)
            persona_config = load_persona()
            emotion = self.emotion_system.detect_emotion(message)

            # สร้าง prompt ด้วย orchestrator
            # Prepare readable memory context
            context_string = "\n".join(
                [f"- {m.get('message', '')}" for m in memory_context]) if memory_context else ""

            prompt = build_betty_prompt_v4(
                message, context_string, persona_config, emotion)
            from betty_emotion_temperature_mapper import \
                map_emotion_to_temperature
            map_emotion_to_temperature(emotion)
            from gpt_with_emotion_control import gpt_reply_with_emotion

            gpt_response = gpt_reply_with_emotion(prompt, emotion=emotion)

            # บันทึกการสนทนา
            self.memory_core.store_user_interaction(
                message, gpt_response, emotion)

            return jsonify({"response": str(gpt_response),
                            "memory_stats": self.memory_core.get_memory_summary()})

        @self.app.route('/api/memory', methods=['GET'])
        def get_memory_stats_alias():
