...
def get_context_summary(
        ...
        limit=5):
    """รวมข้อความจาก stack และ extended context มาเป็น string เดียว"""
    import json
    import os

    stack_path = f"{memory_base_path}/stack/{user_id}_stack.json"
    extended_path = f"{memory_base_path}/extended/{user_id}_context.json"
    lines = []
    ...
    if os.path.exists(extended_path):
        try:
            with open(extended_path, 'r', encoding='utf-8') as f:
                ...
                for m in data:
                    lines.append(
                        f"User: {m.get('message', '')}\nBetty: {m.get('response', '')}")
        except BaseException:
            pass
