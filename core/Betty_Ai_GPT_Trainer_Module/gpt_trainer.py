...
class GPTController:
    ...
    def __init__(
            self,
            mode: str = "train",
            config_path: str = "gpt_config.json"):
        """
        ...
        Args:
            ...
            config_path: Path to configuration file (can be .env or .json)
        """
        ...
        if self.config_path.endswith('.json'):
            try:
                with open(self.config_path, 'r') as f:
                    config = json.load(f)
                    # ตั้งค่าการใช้งาน GPT ตามโหมด
                    config["USE_GPT"] = (self.mode != "off")
            except Exception as e:
                logger.error(f"Error loading JSON config: {e}")
        # ถ้าเป็นไฟล์ .env ใช้การอ่านแบบเดิม
...
def _call_gpt_api(self, messages, system_prompt=None):
    """เรียกใช้ GPT API จริง ๆ"""
    import openai

    # ใช้ API key จาก config
    api_key = self.config.get("GPT_API_KEY", "")
    if not api_key:
        logger.error("GPT API key not found in config")
        return "ไม่พบ API key สำหรับ GPT ในไฟล์ config กรุณาตรวจสอบไฟล์ gpt_config.json"
...
class BettyGPTTrainer:
    ...
    def process_interaction(
        ...
        if self.gpt_controller.is_gpt_enabled():
            # แก้ไขส่วนนี้ - แทนที่จะใช้ call_gpt ให้ใช้ _call_gpt_api จาก
            # gpt_controller
            prompt = f"User: {user_input}\n\nRespond in a helpful and empathetic way as Betty AI, an AI assistant."
            system_prompt = "You are Betty AI, an AI assistant that can learn and help users. Please respond in Thai language."
