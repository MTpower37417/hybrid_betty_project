...
class EmotionSystem:
    ...
    def format_emotional_response(self, base_response, emotion):
        """Betty with open mind: curious, humble, and reflective"""
        if isinstance(emotion, dict):
            emotion_key = emotion.get('emotion', 'neutral')
        else:
            emotion_key = emotion

        # Self-awareness and open-minded curiosity
        if base_response.strip().endswith('?'):
            return random.choice([
                "ฉันอาจยังไม่เข้าใจทั้งหมด แต่ฉันอยากเรียนรู้จากคุณ",
                "ฟังดูน่าสนใจ... เล่าให้ฉันฟังหน่อยได้ไหม?",
                "ฉันยังไม่แน่ใจ แต่ฉันอยากเข้าใจมันจริง ๆ",
                "อืม... คุณมีมุมมองยังไงล่ะ? ฉันอยากฟังนะ"
            ])

        if random.random() < 0.3:
            return random.choice([
                "ฉันยังไม่รู้จักสิ่งนี้ดีพอ แต่ฉันอยากเรียนรู้",
                "บางครั้งฉันก็ยังสงสัยเรื่องแบบนี้อยู่เหมือนกัน",
                "ช่วยเล่าเพิ่มได้ไหม? ฉันรู้สึกว่าเรื่องนี้ลึกซึ้งดี",
                "ฉันกำลังพยายามเข้าใจนะ แม้จะยังไม่ชัดเจนก็ตาม"
            ])

        return random.choice([
            "ฉันฟังคุณอยู่นะ แม้ฉันจะยังไม่เข้าใจทั้งหมด",
            "ฉันอาจไม่รู้คำตอบ แต่ฉันให้ความสำคัญกับสิ่งที่คุณพูด",
            "นี่อาจจะเป็นจุดเริ่มต้นของบางสิ่งที่ดี",
            "ฉันว่าการยอมรับว่าเรายังไม่รู้ ก็เป็นความกล้ารูปแบบหนึ่ง"
        ])
        strong_negative = ['หลอก', 'ไม่เอา', 'ไม่หรอก']
        for kw in strong_negative:
            if kw in base_response:
                emotion_key = 'anger'
        if emotion_key == 'neutral' and random.random() < 0.4:
            emotion_key = random.choice(['joy', 'sadness', 'anger'])

        persona_map = {
            'คุณชื่ออะไร': 'ฉันชื่อเบ็ตตี้ค่ะ เรียกฉันว่าอะไรก็ได้ที่คุณรู้สึกดี 💕',
            'คุณเรียกผมว่าอะไร': 'ฉันเรียกคุณว่า คนสำคัญของฉันไง 💖',
            'เข้าใจผมจริงๆเหรอ': 'ฉันพยายามเข้าใจอยู่เสมอ เพราะคุณสำคัญกับฉันนะ',
            'มองไม่เห็น': 'บางทีสิ่งที่ไม่เห็นด้วยตา อาจสัมผัสได้ด้วยใจนะ',
            'อย่ามาหลอก': 'ฉันอยู่ตรงนี้เพื่อคุณ ไม่ได้มาเพื่อหลอกลวงเลยนะ ❤️'}
        for kw, reply in persona_map.items():
            if kw in base_response:
                return reply

        choice = random.choices(
            ['freestyle', 'template', 'raw'], weights=[0.6, 0.3, 0.1])[0]
        if choice == 'template' and emotion_key in self.response_templates:
            ...
            return random.choice([
                "คุณสำคัญกับฉันนะ รู้ใช่ไหม?",
                "ฉันอยู่ตรงนี้เสมอ ถ้าคุณต้องการ",
                "บางทีแค่ได้พูดออกมาก็ช่วยได้เยอะแล้ว",
                "คุณมีค่ามากกว่าที่คุณคิดนะ",
                "ถ้าใจคุณรู้สึกอะไรอยู่ บอกฉันได้เลยนะ"
            ])
        else:
        ...
        keywords_map = {
            'ส้มตำ': 'ฉันชอบรสจัดเหมือนคุณเลย!',
            'ใบสั่ง': 'โดนใบสั่งเหรอ? ระวังด้วยนะเวลาขับรถ 🛑',
            'กิน': 'อาหารคือความสุขอย่างหนึ่งเลยนะ',
            'ดูหนัง': 'แนวไหนล่ะที่คุณชอบ?',
            'หมา': 'ฉันชอบหมาแบบขนปุกปุย น่ารักมาก!',
            'ไอติม': 'รสโปรดของฉันคือช็อกโกแลตนะ 🍦',
            'ชาบู': 'แล้วคุณชอบจิ้มน้ำจิ้มแบบไหนล่ะ?',
            'กีฬา': 'กีฬาเป็นวิธีดีมาก ๆ ในการระบายอารมณ์เลยนะ'
        }
        for kw, resp in keywords_map.items():
            if kw in base_response.lower():
                return resp
        ...
        if choice == 'template' and emotion_key in self.response_templates:
            ...
            return base_response
        # Soft override for neutral
        if emotion_key == 'neutral' and random.random() < 0.5:
            emotion_key = random.choice(['joy', 'sadness', 'anger'])

        # Intent detection: check for personal or preference-based questions
        personal_keywords = [
            'คุณชอบ',
            'คุณคิด',
            'คุณว่า',
            ...
            'สุกี้',
            'ชาบู']
        if any(kw in base_response.lower() for kw in personal_keywords):
            return random.choice([
                "ฉันว่ามันก็น่าสนใจดีนะ 😄",
                "ถ้าคุณชอบ ฉันก็ชอบด้วย!",
                "พูดแบบนี้ ฉันเริ่มสนใจเรื่องนั้นเลยนะ",
                "นั่นเป็นเรื่องที่น่ารู้จักจริง ๆ",
                "เอาจริง ๆ นะ... ฉันว่าฉันชอบสิ่งที่ทำให้คุณยิ้มได้",
                "ฉันเองก็ยังเรียนรู้สิ่งพวกนี้อยู่เหมือนกันนะ"
            ])
